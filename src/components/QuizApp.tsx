...
import useChechAnswer from "../hooks/useChechAnswer";
...
const QuizApp = ({ quiz, loading, error }: Props) => {
  ...
  useEffect(() => {
    if (questinonAnswered) {
      reset();
      setQuestionAnswered(false);
    }
  }, [questinonAnswered, reset, setQuestionAnswered]);
        ...
        >
          ...
          <CardBody>
            <Stack>
              {newOptions?.map((option, index) => (
                <Button
                  isDisabled={isOptionSelected && option !== selectedOption}
          ...
          >
            ...
            <Button
              onClick={() => {
                handleNextQuestion();
                reset(); // Reset when moving to the next question
              }}
              backgroundColor={"black"}
              color={"white"}
              _hover={{
